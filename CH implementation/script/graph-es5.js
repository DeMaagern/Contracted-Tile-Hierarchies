...
function () {
  ...
  _createClass(Graph, [{
    ...
    key: "addEdge",
    value: function addEdge(newEdge) {
      ...
      if (this.nodes[newEdge.from] && this.nodes[newEdge.to]) {
        ...
        */
        ...
        // B <- A (R)
        var reverseEdge = new Edge(newEdge.to, newEdge.from, newEdge.weight); // Add the forward edge to the source and set it neighbour
        // A -> B
        ...
        this.nodes[reverseEdge.from].edges.reverse[reverseEdge.id] = reverseEdge.weight; // Set as neighbour? Not sure of this
        //this.nodes[reverseEdge.from].neighbours[reverseEdge.to] = reverseEdge.id;

        this.edges[reverseEdge.id] = reverseEdge; // If the graph is undirected, we need to add a forward edge to the destination and a reverse edge to the source

        if (!this.directed) {
          // Add forward edge from the original destination
          // B -> A
          this.nodes[newEdge.to].edges.forward[newEdge.id] = newEdge.weight;
          this.nodes[newEdge.to].neighbours[newEdge.from] = reverseEdge.id; // A <- B (R)
    ...
    value: function removeEdge(edgeId) {
      ...
      if (this.edges[edgeId]) {
        ...
        delete this.nodes[this.edges[edgeId].from].edges.forward[edgeId]; // If the graph is undirected, delete the reverse edge aswell
    ...
    key: "ShortestPath",
    value: function ShortestPath(startId, endId, nodeToSkip) {
    ...
    key: "ShortestPathBiDirectional",
    value: function ShortestPathBiDirectional(startId, endId, nodeToSkip) {
      ...
      newEndNode.edges.reverse = this.nodes[endId].edges.reverse; // Add a starting total weight to the starting node
      ...
      var ownDiscovered = startDiscovered; //console.log("Startqueue length: " + queue.length);
    ...
    key: "calculateUndirectedEdgeDifference",
    value: function calculateUndirectedEdgeDifference() {
      ...
      for (var nodeId in this.nodes) {
        ...
        var alreadyPresent = 0; // For all neighbours
    ...
    key: "calculateDirectedEdgeDifference",
    value: function calculateDirectedEdgeDifference() {
      console.error("calculateDirectedEdgeDifference: Not implemented exception.");
    }
  }, {
    key: "contractGraphUndirected",
    value: function contractGraphUndirected() {
      ...
      while (counter > 0) {
        ...
        console.log("Contracting " + topNode.id + " with an edge difference of " + topNode.edgeDifference); // If the node has been contracted before, skip. Else set as contracted
        ...
        console.log(topNode); // For each neighbour a of the node n that we want to contract
        ...
        for (var firstNeighbour in topNode.neighbours) {
          ...
          for (var secondNeighbour in topNode.neighbours) {
            ...
            if (shortestDistance > weightThroughN) {
              console.log("shortest path(" + shortestDistance + ") is longer than weightThroughN( " + weightThroughN + " )"); // The witness path is longer than the connection. We introduce a shortcut
              ...
              if (this.nodes[firstNeighbour].neighbours[secondNeighbour] != undefined) {
                console.log("Shortcut already exsists"); // We have 2 edges. The forward and backward edge, because this is an undirected graph
                ...
                console.log("Adding shortcut"); // Else, the connection does not exist yet.
              ...
              this.edges[this.nodes[secondNeighbour].neighbours[firstNeighbour]].contracts = topNode.id; // We still need to update the neighbours, because the layout changed.
...
function () {
  ...
  _createClass(Node, [{
    key: "copyContructor",
    value: function copyContructor(node) {
      this.id = node.id;
      this.x = node.x;
      this.y = node.y;
