...
async function getClosestNode(latlng) {
  ...
  $.ajax({
    ...
    success: function (data, status, xhr) {
      //console.log("Succes!");
      var nodes = parseJson(data, 14).nodes; // Get closest node
      ...
      if (!node1) {
        ...
        L.circle([node1.y, node1.x], {
          radius: 50
        }).addTo(markerLayer);
      } else if (!node2) {
        node2 = JSON.parse(JSON.stringify(closestNode)); // Do all code for route calculation

        L.circle([node2.y, node2.x], {
          radius: 50
        }).addTo(markerLayer); // Fetch all tiles from start & finish untill they meet
...
async function reconstructGraph(node1, node2) {
  var zoomlevel = 14; // Get tile ID's
...
function drawBoundingBox(x, y, z) {
  ...
  var x2 = tile2long(x + 1, z); //console.log(y1 + " - " + x1 + " - " + y2 + " - " + x2);

  L.polyline([[y1, x1], [y1, x2], [y2, x2], [y2, x1], [y1, x1]], {
    color: colour[z]
  }).addTo(mymap);
}
...
function addingFinished() {
  if (tilesFetched == totalTiles) {
    reconstructTime = Date.now(); // Do shortest path search
  ...
  jsonData["@graph"].forEach(function (elem) {
    ...
    if (elem["geo:long"] || elem["@type"] == "osm:Node" || elem["@type"] == "osm:node") {
      ...
      for (var i = 0; i < nodesOnWay.length - 1; i++) {
        ...
        if (result.edges[elem["@id"]]) {//console.log("Edge already exists");
        }
